...
It is recommended that you enable Supabase's Row Level Security [(RLS)](https://supabase.com/docs/guides/auth/row-level-security) for your database tables. To access the contents of a database table from within your app, you must add an RLS policy to allow your database to be read:
...
1. Select **For full customization** to create your own policy.
    ...
    CREATE POLICY "Enable read for authenticated visitors" ON "public"."your_table"
    ...
    <Callout type="info">
      ...
      This policy gives full read access on your table to _any_ visitor to your app who authenticates through Clerk. To limit each user's access to only their data, see [the docs on accessing user IDs in RLS policies](/docs/integrations/databases/supabase/rls-policies).
